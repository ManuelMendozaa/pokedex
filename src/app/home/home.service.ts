...
export class HomeService {
  ...
  private pokemonUrl: any = 'assets/pokemon.json';
  pokemonDetalle;
  ...
  pokeDetalle(pokemon: Object){
    
    this.pokemonDetalle = pokemon;
    
  }

  ObtenerImagenEvolucion(nombre:string){
    var pokemons = this.getPokemon();
    for(let i = 0; i < pokemons.length; i++){
      if(pokemons[i].name == nombre){
        return pokemons[i].art_url;
      }
    }
  }
}
